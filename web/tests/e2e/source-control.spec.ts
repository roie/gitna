...
test('Gitna Home searches recent folders and protects dirty drafts', async ({ page, app }) => {
  ...
  const homeSurface = page.locator('main[aria-labelledby="gitna-home-title"]')
  const homeBack = page.getByRole('button', { name: `Back to ${basename(app.repo)}` })
  ...
  expect(homeLogoBox!.y).toBeLessThanOrEqual(128)
  const homeBackBox = await homeBack.boundingBox()
  expect(homeBackBox).not.toBeNull()
  expect(homeBackBox!.y + homeBackBox!.height).toBeLessThan(homeLogoBox!.y)
  await expect(homeSurface).toHaveCSS('overflow-y', 'auto')
  ...
  const recentList = recentFolder.locator('xpath=ancestor::ul')
  await expect(recentList).toHaveCSS('overflow-y', 'visible')
  await expect(recentList).toHaveCSS('max-height', 'none')
  await expect(recentList).not.toHaveClass(/gitna-scrollbar/)
  const lastOpened = recentFolder.locator('time')
  await expect(lastOpened).toContainText(/Just now|minute(?:s)? ago/)
  await expect(lastOpened).toHaveAttribute('title', /^Last opened /)
  await expect(lastOpened).toHaveAttribute('aria-label', /^Last opened /)
